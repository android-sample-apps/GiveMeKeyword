...
class FirebaseDB(val activity: Activity) {
    ...
    public fun saveDrawing(keyword:String,image: Bitmap,name:String,content:String) : Boolean{

        val db = FirebaseFirestore.getInstance().collection(keyword)
        val doc = db.document() //고유 id를 자동으로 생성

        val storage = FirebaseStorage.getInstance()
        val storageRef = storage.reference  //storage 참조
        val imagesRef: StorageReference? = storageRef.child("images/"+doc.id+".jpg")
